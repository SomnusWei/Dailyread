...
- 📄 **讲义分发**：教师/管理员上传 HTML 讲义并按学生等级分发；支持 12 个分类筛选（基础学｜诊断学｜针灸腧穴｜中药｜方剂｜内科｜外科｜妇科｜儿科｜推拿｜养生｜经典）；学员点击在线阅读；已分发讲义可**追加分发**（追加等级或指定账号，自动向未收到通知的账号补发通知且去重不重复打扰）；讲义可见性按等级动态匹配，新建账号可查看其等级范围内的历史讲义；「分发讲义」按钮以模态弹窗形式打开（分发表单 + 我的分发记录），不在列表下方展开
