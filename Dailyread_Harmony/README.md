...
> 配套 Windows 端的文章&概念管理器：https://github.com/SomnusWei/article-concept-manager
...
**🐛 Bug 修复：必读文章跨月后不再出现在任务列表**

- **问题**：在7月1日完成必读文章打卡后，完成率变为100%，导致7月2日、7月3日必读文章不再出现在任务列表中
- **根因**：`filterTaskPool()` 方法只检查 `completionRate < effectiveRate` 的条件，必读文章被错误地过滤掉了
- **修复方案**：修改 `DailyTaskService.ets` 中 `filterTaskPool()` 方法，必读文章（`isRequired = true`）始终进入任务池，不受完成率限制

**🐛 Bug 修复：必读文章计入每日字数限制**

- **问题**：必读文章的字数被计入每日阅读字数目标，导致实际阅读量超出用户设定
- **修复方案**：
  - 修改 `DailyTaskService.ets`：计算任务总字数时排除必读文章
  - 修改 `Home.ets`：计算今日阅读字数时排除必读文章
