# 中医讲义质量检查清单

> 分级检查：P0（致命）→ P1（重要）→ P2（一般）
> **讲义默认交付 OneNote 适配的 HTML（不输出 PDF）**，故除下列通用项外，**必须**通过
> 第八节「OneNote 版专属检查」全部 12 项。格式规范见 `references/onenote-html-spec.md`。

---

## P0 级（致命错误，必须修复）

### 内容准确性
- [ ] 每个证型的"症状→病机→治法→方剂"逻辑链条自洽
- [ ] 所有方剂组成完整，君、臣、佐、使标注正确
- [ ] 证型分类与教材一致，无遗漏或错配
- [ ] 治则治法与证型病机对应正确
- [ ] 含毒性药物（附子、川乌、草乌、朱砂、雄黄、细辛、马钱子等）必须标注使用注意和禁忌
- [ ] 无重大医学常识错误（如虚实错辨、寒热颠倒）
- [ ] 疾病概念定义准确，与教材一致

### 格式正确性
- [ ] HTML 标签正确闭合（`</body></html>` 完整）
- [ ] 无未闭合的 `<div>`、`<table>`、`<ul>` 等标签
- [ ] 文件编码为 UTF-8，无乱码
- [ ] **OneNote 版已生成**（`<讲义名>_OneNote版.html` 存在且非空）——缺此文件即 P0

---

## P1 级（重要问题，应该修复）

### 结构完整性
- [ ] 讲义结构完整：封面 + 目录 + 总论 + 各论 + 附录
- [ ] 目录条目与正文章节一一对应，无缺漏、无多余
- [ ] 总论包含：脏腑生理、病因病机总览、辨证治疗原则
- [ ] 各论每病包含：理（概念+病因病机+诊断鉴别）、法（治则）、方药（证型+方剂+方解+药物）
- [ ] 附录包含：方剂速查表、药物速查表
- [ ] 每章末尾有小结

### 知识准确性
- [ ] 每味药物的性味归经准确
- [ ] 药物功效主治描述准确
- [ ] 病因病机分析符合中医理论
- [ ] 诊断与鉴别诊断要点清晰准确
- [ ] 交叉引用正确（"详见XX病XX证"指向正确位置）

### 排版规范性
- [ ] 表格内容完整，无错位、无断行
- [ ] 卡片（教材基线 / 专家 / 记忆框架 / 考点边界 / 临床联系 / 提示）无断裂
- [ ] 无大块空白区域
- [ ] **OneNote 版结构统计与网页版一致**：卡片数、引文数、表格数、各级标题数逐项核对

### 交叉引用
- [ ] 方剂速查表包含所有正文中出现的方剂
- [ ] 药物速查表包含所有展开详解的药物
- [ ] 同一方剂在不同疾病中首次出现完整讲解，后续有引用说明
- [ ] 跨系统疾病有明确的引用说明

### 专家内容合规
- [ ] 专家引用带**可读来源标注**（如「蒸馏Skill·专家名·第N讲」／「《石学敏针灸全集 第2版》p12」）
- [ ] **交付正文不出现时间戳**：无 `P## [hh:mm:ss]`、其区间形式（如 `P09 [00:12:28–00:12:45]`），
      **也无「无讲次前缀的续段」**（如 `刘忠保·第 9 讲 / [00:39:38–00:42:15]` 的后半段），
      **也无 `P## [hh:mm:ss]` 这类记法示例**——蒸馏专家的课程时间戳仅作内部溯源，
      不得进入笔记／讲义／题目等任何交付内容。
      验证：`python scripts/strip_timestamps.py --in <文件>.html --dry-run --check`，残留应为 0
- [ ] 专家观点以独立卡片与教材并列，标注来源，不替代教材结论
- [ ] 专家"考不考"类判断附适用边界声明

---

## P2 级（一般问题，酌情修复）

### 排版细节
- [ ] 标点符号规范统一（全角标点）
- [ ] 字号、间距、缩进一致
- [ ] 颜色使用符合主题色系
- [ ] 无多余空行和空格
- [ ] 术语表述统一（如"咳喘"vs"喘咳"、"证候"vs"证型"）
- [ ] 目录层级缩进正确（分组 / 一级 / 二级三级样式区分）
- [ ] 表格对齐整齐
- [ ] 序号格式统一

### 文字细节
- [ ] 无明显OCR错误和错别字
- [ ] 药名、方名无错字（如黄芩≠黄苓、白术≠白木、炙甘草≠灸甘草）
- [ ] 引用原文无遗漏
- [ ] 数字和单位格式统一

---

## OneNote 版专属检查（讲义必检）

> 1–7、11 项由 `python scripts/to_onenote.py --src 讲义.html --out 讲义_OneNote版.html --check` 自动完成；
> **方剂学／生理学／中药学体例**（直接按体例蓝本原生形态编写）用 `--check-only <交付物>` 完成 1–7 项；
> 8–10 项需用脚本逐句核对。

- [ ] 1. 无 `class` 属性残留（`class="` 计数 = 0）
- [ ] 2. 无 CSS 变量残留（`var(--` 计数 = 0）
- [ ] 3. 无渐变残留（`linear-gradient` 计数 = 0）
- [ ] 4. 无浮动 / 粘性定位（`position:sticky`、`position:fixed` 计数 = 0）
- [ ] 5. 无伪元素残留（`::before`、`::after` 计数 = 0）
- [ ] 6. 无 px 字号残留（`px"` 计数 = 0）
- [ ] 7. 标签配对平衡（`table/tr/td/th/p/span/ul/ol/li/h2/h3/h4` 开闭计数相等）
- [ ] 8. 结构统计与网页版一致（卡片 / 引文 / 表格 / 标题数逐项核对）
- [ ] 9. 内容零丢失：源文逐句在产出中可检索到（唯一允许差异＝被替换掉的侧栏目录）
- [ ] 10. 无新增内容：产出中无源文没有的实质内容
- [ ] 11. 文内目录非空（脚本未报"未解析到目录条目"）
- [ ] 12. 封面主标题、章节标题的**全角空格（U+3000）未被折叠**

---

## 体例检查（按 SKILL.md「讲义体例路由」择一执行）

### 方剂学体例检查（`references/lecture-format-fangjixue.md`）

- [ ] 三篇完备：上篇·总论（四节）／中篇·各论／下篇·附录（五个）
- [ ] 总论含「立法依据与适应范围」「分类与代表方（表）」「使用注意（含 ⚠️ 禁忌速记）」「专家总论（双专家卡）」
- [ ] **每首方剂严格按七层**：① 组成用法 ② 功用主治 ③ 证治机理 ④ 配伍结构（君臣佐使）⑤ 剂量使用讲解（原方—教材—倪师临床）⑥ 附方 ⑦ 运用与鉴别
- [ ] 每方之后附**倪海厦**与**邓中甲**两张方义解析卡（无专论者标题标注「推演」，不假托原话）
- [ ] 附录三速查表覆盖正文**全部**方剂（正方 + 附方）；附录一、二覆盖正文全部中药
- [ ] 附录四为三列剂量对照表，附录五为专家内容使用说明与边界声明（含免责声明）
- [ ] 专家标注符合体例：倪海厦标「倪海厦观点」；**邓中甲标「邓中甲」，不标「蒸馏Skill」**（讲稿整理本，非仓颉蒸馏产出）
- [ ] 篇分隔（`中 篇 · 第 一 章 …`／`下 篇 附 录`）、`▍` 前缀、主色 `#4a1a10`／表头 `#7a2e1e` 与蓝本一致

### 生理学体例检查（`references/lecture-format-physiology.md`）

- [ ] 三段式完备：`［目标］`／`［第一节］…［第N节］`／`［速查］`
- [ ] `［目标］` 含「考点边界速览」+ 教材基线卡 + 专家卡（专家｜一句话定调｜出处）+ 专家原话引文块
- [ ] 每节含教材基线定位卡，节内按 `▍一、…／▍二、…` 分层
- [ ] `［速查］` 含速查表（含一张**专家口诀汇总**表）+ 本章小结 + `▍章末思考题（教材原题）` + 使用说明
- [ ] 专家内容标注「蒸馏Skill·刘忠保·基于有限素材」；考点判断附适用边界声明
- [ ] 封面 kicker（`PHYSIOLOGY · LECTURE 0N`）／23pt 章名／主题行／来源三行齐全
- [ ] 主色 `#143561`／表头 `#1e4d8c`／专家卡 `#b8860b` 配色与蓝本一致

### 中药学体例检查（`references/lecture-format-zhongyaoxue.md`）

- [ ] 十节完备：一、药材与来源／二、药性（性味归经）／三、功效／四、应用／五、用法用量与炮制／六、使用注意／七、现代研究／八、专家视角／九、类药鉴别／十、速查表与小结
- [ ] **章标题不带 ［…］ 标签**：全文 `［`／`］` 计数 = 0、`class="num"` = 0（本类体例专属，必查）
- [ ] 第一节含教材基线卡（来源含拉丁学名与药用部位／主产／采收加工／性状／炮制）
- [ ] 第二节含"由功效反推性能"专家卡 ＋ 记忆框架表（`功效 ｜ 对应主治 ｜ 推出性能`）
- [ ] 第三节含考点边界卡：教材口径与专家口径并列，并注明"考试以教科书为准"
- [ ] 第四节每个主治独立 `h3`，各含教材基线卡（含代表方表）＋专家卡；末设「此外」
- [ ] 第五节炮制表**含「饮片形态（认药要点）」列**
- [ ] 第六节为警示卡；特殊药附剂量与慎用/禁用人群（麻黄：运动员禁用，高血压、失眠慎用）
- [ ] 第七节含药典含量指标；专家"五步取证"式论证做成 `取证步骤 ｜ 证据` 表
- [ ] 第八节每位专家独立卡 ＋ 使用说明卡（取材／标注规则／边界声明／免责声明 四段齐全）
- [ ] 第九节鉴别表覆盖 本药 ＋ 同类 4 味 ＋ 易混药，并含易错点卡（2～3 条）
- [ ] 第十节一览速查表覆盖 来源/药性/功效/主治/此外/用法/注意/关键成分/个性特征
- [ ] 饮片图注如实：非实拍须写"示意图，非实拍"；未把同一张图重复标成不同饮片
- [ ] **配色与药性一致**：主题色／主题深色／教材基线卡三色／小结框两色／隔行底纹／专家卡四色／引文块字色，全部取自体例规范 **5.1 药性配色表**的**同一档**（寒 `#23557d`／凉 `#1a6f69`／平 `#8a5b00`／温 `#ab5f13`／热 `#b03a26`）——无混档、无他档残留；语义卡色（警示／记忆框架／提示／临床联系／大字块）保持固定
- [ ] 正文引号全角配对（`“` 与 `”` 计数相等），属性值内无英文双引号


### 中医内科学体例检查（`references/lecture-format-neike.md`）

- [ ] 章结构完备：封面表 → 目录表 → ［第N章］ → 章首概要 → 各节 → ［附录一］～［附录四］
- [ ] **章标题带 `［第N章］`／`［附录N］` 标签**（本类体例专属，与中药学相反，必查）
- [ ] 每个 `h2` 下有一条 3px 主题色分隔条（`height:3px;line-height:3px;font-size:1pt;`）
- [ ] 每节 `h3` 与每层 `h4` 均带 `▍` 前缀；节名形如 `▍第一节　感　　冒`（病名二字间加全角空格）
- [ ] **目录只到两级**：`padding-left:14px` 的节下子项行计数 = 0（不得出现 `· 辨病 · 病因 · 病机分析 · 辨证论治 · 临证备要 · 专家视角`）
- [ ] **六层不缺项**：每节 h4 依次为 （一）辨病 →（二）病因 →（三）病机分析 →（四）辨证论治 →（五）临证备要与预防调护 →（六）专家视角
- [ ] **节内固定栏目齐全**：古籍摘要卡 1 ／ 记忆框架卡 1 ／ **高频考点·低频易混双栏** 1 ／ 临床联系卡 2 ／ 三张专家卡各 1 ／ 教材基线卡 8～9（本类体例**无「本节小结」栏**，节末以经方专家库卡收束）
- [ ] **三张专家卡齐全**：每节（六）内 倪海厦 ／ 娄绍昆 ／ 经方专家库 各一张，卡序固定；章首概要节同样三张
- [ ] 每张专家卡含「一句话定调 ＋ 抓手表 ＋ 原话逐字引用表（原话｜出处）」；经方专家库卡另含各家分歧或边界段
- [ ] **附录四不缺**：经方专家库速查表四张卡（七模型／本章病证矩阵／原话精选／使用纪律与边界）
- [ ] 附录一含 h4「甲、核心方剂详解表」与「乙、方名索引（按节次）」；附录二毒性药附剂量上限与煎服法
- [ ] **主题色一致且单档**：主题色族只取 `#014198`（RGB 1,65,152）与 `#012d6a`／`#f3f7fc`／`#c5d6ea`／`#eef4fa`／`#e6eef8`／`#cbdcee`／`#e8eefb`／`#e4eaf6`／`#eef3fb`——**无其他主题色残留**（旧青系 `#0f6b6b`／`#0a4b4b` 计数应为 0）
- [ ] 语义卡色固定：专家卡琥珀 `#b8860b`／警示 `#a83232`／记忆 `#6b4fbb`／提示与使用说明 `#2a6099`／临床 `#1a6b3c`
- [ ] **经方专家库内容点名到人**（谦斋／姚荷生／印会河／赵绍琴／教材／《名老中医之路》某篇），无"经方派认为""中医认为"式表述
- [ ] **引文逐字**：卡片中原话可在对应素材中逐字检出；跨页或插叙处用「……」标示并加注
- [ ] 附录三「专家内容**三个**来源」齐全（倪海厦／娄绍昆／经方专家库八部著作），并含免责声明与病种特殊提示（肺痨抗结核为主、急危重证须急诊）
- [ ] 正文引号全角配对；文末有 `— 中医内科学·<章名>讲义　完 —` 页脚行


### 内容零丢失核对法

```python
# 源文按中文句读切分，逐句在产出中检索；再反向检查产出有无新增句
DROP = r'[［］「」·▍\s]'
# 正句：源句子（去 DROP）必须出现在产出（去 DROP）中
# 反句：产出中长度 ≥30 的句子（去 DROP）必须出现在源文中
# 首页的侧栏目录整体被文内目录替换，属预期差异，单独豁免
```

---

## 快速验证命令（PowerShell）

```powershell
# ========== 格式检查 ==========

# 1. 检查HTML标签闭合
$htmlPath = "讲义_OneNote版.html"

# 检查结束标签
$bodyClose = Select-String -Path $htmlPath -Pattern "</body>"
$htmlClose = Select-String -Path $htmlPath -Pattern "</html>"
Write-Output "body闭合: $($bodyClose.Count), html闭合: $($htmlClose.Count)"

# 2. OneNote 版残留物检查（7 项应全为 0）
$forbidden = @('class="', 'var(--', 'linear-gradient', 'position:sticky', '::before', '::after', 'px"')
foreach ($pat in $forbidden) {
    $c = (Select-String -Path $htmlPath -Pattern ([regex]::Escape($pat))).Count
    Write-Output "$(if ($c -eq 0) {'OK '} else {'!! '})残留 '$pat': $c"
}

# 3. 标签配对
foreach ($tag in @('table','tr','td','th','p','span','ul','ol','li','h2','h3','h4')) {
    $o = (Select-String -Path $htmlPath -Pattern "<$tag[ >]").Count
    $c = (Select-String -Path $htmlPath -Pattern "</$tag>").Count
    if ($o -ne $c) { Write-Output "!! $tag 不匹配: open=$o close=$c" }
}

# 4. 检查文件末尾
Write-Output "=== 文件末尾5行 ==="
Get-Content $htmlPath -Tail 5

# 5. 检查常见OCR错字
$typos = @("黄苓", "白木", "灸甘草", "羌话", "蒿本", "伏苓", "半厦", "构杞", "黄茋")
foreach ($typo in $typos) {
    $matches = Select-String -Path $htmlPath -Pattern $typo
    if ($matches) {
        Write-Output "⚠️  可能有错字 '$typo': $($matches.Count)处"
    }
}

# 6. 检查毒性药物是否标注使用注意
$toxicHerbs = @("附子", "川乌", "草乌", "朱砂", "雄黄", "细辛", "马钱子")
foreach ($herb in $toxicHerbs) {
    $count = (Select-String -Path $htmlPath -Pattern $herb).Count
    if ($count -gt 0) {
        Write-Output "ℹ️  含毒性药物 '$herb' 共 $count 处，请确认是否标注了使用注意"
    }
}

# 7. 检查文件大小
$file = Get-Item $htmlPath
Write-Output "`n文件大小: $([math]::Round($file.Length/1KB, 1)) KB"
```

> 更省事的做法：直接跑 `python scripts/to_onenote.py ... --check`，第 1–3 项它一次全查完。

---

## 人工检查要点

### 快速通读（10分钟）
1. 翻一遍文内目录，确认章节完整、三级层级正确
2. 随机打开3-5个章节，检查结构是否完整
3. 查看附录速查表，确认无明显遗漏
4. 用浏览器打开 OneNote 版，确认卡片底色、表格隔行底色、时间戳高亮均正常
5. 在 OneNote 里粘贴一小段试一下，确认表格与配色能保留

### 重点抽查（20分钟）
1. 选1个疾病，通读全章，检查"理-法-方-药"逻辑是否通顺
2. 选2首方剂，核对组成和方解是否正确
3. 选3味中药，核对性味归经和功效
4. 检查所有含毒性药物的段落，确认有使用注意

---

## 常见错误速查

| 错误类型 | 示例 | 检查方法 |
|----------|------|----------|
| OCR错字 | 黄芩→黄苓、白术→白木、炙→灸 | 搜索常见错字列表 |
| 方剂组成遗漏 | 少了佐药或使药 | 对照方剂学教材核对 |
| 性味归经错误 | 性寒写成性温 | 对照中药学教材核对 |
| 证型张冠李戴 | 把A病的证型写到B病 | 对照中医内科学核对 |
| 治法与证型不对应 | 阴虚证用温阳法 | 通读每证"症状→治法→方剂"逻辑链 |
| 标签未闭合 | 缺少 `</table>` | PowerShell 统计开闭标签数 / `to_onenote.py --check` |
| **OneNote 版漏转** | 手写 OneNote 版导致格式不一致 | 必须用 `scripts/to_onenote.py`，禁止手写 |
| **全角空格被吞** | 标题"第一章　绪论"变成半角 | 转后检查 U+3000 是否仍在 |
| **目录层级错位** | 二级条目未缩进、分组标题带了项目符号 | 对照 `onenote-html-spec.md` 第五节 |
