...
> 供「模拟题出题」功能使用。支持执业医师考试、考研、跟读自测三类场景；默认**只交付可交互 HTML 试卷（自动批改）**，**答案卷默认不出**——仅当用户明确要求时才生成。
>
> ⚠️ **卷面洁净原则（中医、西医均适用）**：出题大纲、考试场景（执医/考研等）、难度级别等**仅供内部命题取材使用，一律不得出现在试卷上（若另出答案卷，同样不得出现）**——题干前不加任何标记（如【执医】【考研】【简单】等），卷头不显示出题大纲，卷面不标注单题难度。
...
2. **答案卷 HTML**（`--with-answers` 生成，**默认不出**）——题目+答案+解析+出处，打印友好；仅当用户明确要求答案卷／打印版／PDF 时才加该参数
3. **答案卷 PDF**（答案卷 HTML 转 PDF，**可选**）——需先有答案卷；仅当用户明确要求 PDF（或打印版）时转换

> 交互式试卷在提交后已**就地对答案**：客观题自动判分、主观题给参考答案与要点自评，并支持 `?r=1` 成绩回显。
> 因此日常自测**不需要答案卷**；答案卷的实际用途只有两个——**教师/家长留档**与**转 PDF 打印**。
...
```
①确认需求（大纲/题数/总分/场景/难度）→ ②知识库取材 → ③命题 → ④生成试卷（答案卷按需） → ⑤质量自检
```
...
# 仅当用户明确要求答案卷/打印版时，加 --with-answers 一并生成答案卷 HTML
python build_exam_html.py --exam exam.json --output-dir 试卷输出 --name "{试卷名}" --with-answers

# 答案卷转 PDF（可选步骤，需先有答案卷 + 用户明确要求 PDF/打印版）
# 首选 Chrome：
...
产出文件：**默认 1 个**——`{试卷名}.html`（交互试卷）；
加 `--with-answers` 后为 2 个（＋`{试卷名}_答案卷.html`）；
用户明确要 PDF 时为 3 个（再＋`{试卷名}_答案卷.pdf`）。
...
- [ ] 卷面洁净：卷头不出现「出题大纲」（若另出答案卷，答案卷同样不出现）
...
**默认交付：仅交互式 HTML 试卷**（见「标准交付物」）；答案卷默认不出，仅用户明确要求时加 `--with-answers` 生成，PDF 再在其上按需转换。用户仅要 Markdown 版时才输出纯文本卷（答案解析置末）。
